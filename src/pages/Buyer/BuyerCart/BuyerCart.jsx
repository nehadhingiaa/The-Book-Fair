...
import { decreaseQuantity, increaseQuantity, removeItem } from '../Books/OrdersSlice';
...
const BuyerCart = () => {
  ...
  return (
      ...
      <div className={`${cartData.length ? "col-span-9 " :"col-span-12 "} w-full max-w-screen bg-purple-100 shadow rounded`}>
      <div>
      ...
      <thead className="bg-purple-200">
        <tr>
          ...
          <th className="px-4 py-2 text-left border-b text-black">Product</th>
      ...
      <tbody className='overflow-auto'>
        ...
        <tr key={book.id}>
          ...
          <td className="px-4 py-2 border-b"><span>rs.</span> {book.price * book.quantity} <span>/-</span> </td>
          <td className="px-4 py-2 border-b">
            <span className='flex justify-center gap-2'>
              <span className='text-black text-5xl cursor-pointer' onClick={()=>handleDec(book?.id)}>-</span>
              <button className=' border-2 border-purple-300 text-black text-md py-2 px-3'>{book.quantity}</button>
              <span className='text-black text-4xl cursor-pointer' onClick={()=>handleInc(book?.id)}>+</span>
            </span>
          </td>
          <td className="px-4 py-2 border-b">
            <span className='flex gap-2'>
              <button className='bg-purple-200 text-black rounded-md p-2'>View</button>
              <button className='bg-purple-200 text-black rounded-md text-md p-2' onClick={()=>handleRemoveItem(book.id)}>Remove</button>
       ...
       <div className='w-full min-h-screen flex flex-col justify-center items-center  m-auto'>
        <span className='text-9xl'><IoCart/></span>
        <p className='text-4xl'>Your cart is empty</p>
        <p className='text-3xl'>Looks like you have not made any choice yet..</p>
       </div>
    }
      </div>

     {
      cartData.length && <>
       ...
       <PlaceOrder cartData={cartData} subQuantity={subQuantity} subTotal={subTotal}/>
       </div>
      </>
     }
      </div>
